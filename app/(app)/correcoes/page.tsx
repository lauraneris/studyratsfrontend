...
// Dados simulados de redações pendentes
...
// Estado inicial para o formulário de correção
...
export default function CorrecoesPage() {
    ...
    const [selectedSubmission, setSelectedSubmission] = useState(mockSubmissions[0] || null);
    const [correctionForm, setCorrectionForm] = useState(initialCorrectionState);

    const handleSelectSubmission = (submission: any) => {
        setSelectedSubmission(submission);
        // Reinicia o formulário sempre que uma nova redação é selecionada
    ...
    const handleCriteriaChange = (index: number, field: 'score' | 'feedback', value: any) => {
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white neo-card h-full">
                        ...
                        <div className="max-h-[80vh] overflow-y-auto">
                            {submissions.map((sub) => (
                                <div key={sub.id} onClick={() => handleSelectSubmission(sub)}
                                    className={`p-4 cursor-pointer border-b-2 border-gray-200 hover:bg-gray-100 ${selectedSubmission?.id === sub.id ? 'bg-blue-50' : ''}`}>
                                    <p className="font-black text-brand-black">{sub.studentName}</p>
                                    <p className="text-sm font-sans text-gray-600 truncate">{sub.themeTitle}</p>
                                    <p className="text-xs font-sans text-gray-500">{new Date(sub.submissionDate).toLocaleDateString('pt-BR')}</p>
                                </div>
                            ))}
                            {submissions.length === 0 && <p className="p-4 text-center font-sans">Nenhuma redação pendente!</p>}
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    {selectedSubmission ? (
                        <div className="bg-white neo-card">
                            <div className="p-4 border-b-4 border-brand-black">
                                <h2 className="text-xl font-black text-brand-black">A Corrigir: {selectedSubmission.studentName}</h2>
                                <p className="font-sans text-sm text-gray-600">{selectedSubmission.themeTitle}</p>
                            </div>
                            <div className="p-6 max-h-[75vh] overflow-y-auto space-y-6">
                                ...
                                <form onSubmit={handleSubmitCorrection} className="space-y-4">
                                    <div>
                                        <label className="font-black text-lg">Nota Geral (0-1000)</label>
                                        <input type="number" name="overallScore" value={correctionForm.overallScore} onChange={handleFormChange}
                                            className="w-full border-3 border-brand-black h-12 px-4 font-sans text-lg mt-2" />
                                    </div>
                                    <div>
                                        <label className="font-black text-lg">Comentário Geral</label>
                                        <textarea name="generalComment" value={correctionForm.generalComment} onChange={handleFormChange}
                                            rows={5} className="w-full border-3 border-brand-black p-4 font-sans text-lg mt-2 resize-y" />
                                    </div>
                                    {/* Critérios */}
                                    <div className="space-y-3">
                                        {correctionForm.criteria.map((c, index) => (
                                            <details key={c.id} className="border-2 border-brand-black">
                                                <summary className="p-3 font-black cursor-pointer">{c.name}</summary>
                                                <div className="p-4 border-t-2 border-brand-black space-y-2">
                                                    <label className="font-black text-sm">Nota (0-200)</label>
                                                    <input type="number" value={c.score} onChange={(e) => handleCriteriaChange(index, 'score', parseInt(e.target.value) || 0)}
                                                        className="w-full border-2 border-brand-black h-10 px-3 font-sans" />
                                                    <label className="font-black text-sm">Feedback</label>
                                                    <textarea value={c.feedback} onChange={(e) => handleCriteriaChange(index, 'feedback', e.target.value)}
                                                        rows={3} className="w-full border-2 border-brand-black p-3 font-sans resize-y" />
                                                </div>
                                            </details>
                                        ))}
                                    </div>
                                    <button type="submit" className="w-full bg-brand-green text-white neo-button font-black text-xl py-4 hover:bg-brand-blue flex items-center justify-center gap-3">
                        ...
                        <div className="bg-white neo-card p-12 text-center">
                            ...
                            <p className="font-sans text-gray-600">Escolha uma redação na fila à esquerda para começar a correção.</p>
                        </div>
                    )}
                </div>
            </div>
