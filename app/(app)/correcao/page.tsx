...
import { FileText, Upload, Camera, Mic, Send, Coins, Loader2 } from 'lucide-react';

const CORRECTION_COST = 5;

type SubmissionType = 'texto' | 'arquivo' | 'foto' | 'audio';

export default function CorrecaoPage() {
  const [submissionType, setSubmissionType] = useState<SubmissionType>('texto');
    ...
    try {
      ...
      if (user && user.profile) {
        const updatedProfile = {
          ...user.profile,
          stuart_coins_balance: user.profile.stuart_coins_balance - CORRECTION_COST,
        };
  ...
  return (
    <div>
      ...
      <div className="bg-white neo-card p-8 space-y-8">
        <div>
          <label className="block text-xl font-black mb-3 text-brand-black">1. COMO QUER ENVIAR?</label>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <button
                key={type.id}
                onClick={() => {
                  setSubmissionType(type.id as SubmissionType);
                  setFile(null);
                  setTextContent('');
                }}
                className={`p-4 neo-button font-black text-sm flex flex-col items-center justify-center gap-2 h-28 ${
        ...
        <div>
          ...
          {['arquivo', 'foto'].includes(submissionType) && (
            <div className="border-4 border-dashed border-brand-black p-8 text-center bg-gray-50">
              ...
              <label
                htmlFor="file-upload"
                className="bg-brand-blue text-white px-6 py-3 neo-button font-black cursor-pointer inline-block hover:bg-brand-pink"
              >
                ESCOLHER ARQUIVO
              </label>
              {file && (
          ...
          {submissionType === 'audio' && <AudioRecorder onRecordingComplete={handleAudioRecording} />}
        </div>

        {error && <Alert message={error} />}
        {!error && (
          <Alert
            message={`A correção consome ${CORRECTION_COST} StuartCoins e pode levar alguns minutos. Você será notificado quando estiver pronta!`}
          />
        )}
